    ...
    public function build($order, $client)
    {
        ...
        if (iconv_strlen($body_template) == 0) {
            $body_template = trans('texts.order_message',
                ['amount' => $order->product->price, 'account' => $order->account->present()->name()], null,
                $order->task->customer->locale());
        }
        ...
        $this->setTemplate($order->task->customer->getSetting('email_style'))
