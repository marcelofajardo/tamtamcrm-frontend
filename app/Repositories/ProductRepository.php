 ...
 */
public
function listProductAttributes(): Collection
{
 ...
 */
public
function saveProductAttributes(ProductAttribute $productAttribute, Product $product): ProductAttribute
{

    $product->attributes()->updateOrCreate(
        ['product_id' => $product->id], $productAttribute->toArray()
    );
 ...
 */
public
function getProductsByDealValueAndCategory(Category $category, Request $request): Support
{
    return $this->model->join('product_attributes', 'product_attributes.product_id', '=', 'products.id')
        ->join('category_product', 'category_product.product_id', '=', 'products.id')
        ->select('products.*')
        ->where('product_attributes.range_from', '<', $request->valued_at)->where('product_attributes.range_to', '>',
            $request->valued_at)
 ...
 */
public
function deleteFile(array $file, $disk = null) : bool
{
 ...
 */
public
function deleteThumb(string $src) : bool
{
 ...
 */
public
function findProductImages(Product $product) : Collection
{
 ...
 */
public
function saveCoverImage(UploadedFile $file) : string
{
 ...
 */
public
function saveProductImages(Support $collection, Product $product) : bool
{
...
public
function save($data, Product $product) : ?Product
	{
